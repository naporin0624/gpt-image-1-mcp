...
- **Tools crash with `Cannot read properties of undefined (reading 'type')`** when the OpenAI SDK surfaces an error of shape `{ error: undefined, ... }` (instead of `{ error: null }` or a fully populated error object). `handleOpenAIError`'s null guard checked `=== null` only, so the `undefined` case fell through to `errorDetails.type` and crashed. As a result, the **real** API error (e.g. `model_not_found` for accounts that haven't yet been granted gpt-image-2) was masked by the crash, and the SDK's retry policy waited the full timeout × maxRetries before failing (~480s).
- The handler now narrows `errorDetails` defensively (`!= null && typeof === "object" && "type" in errorDetails`), so unknown error shapes fall through to the generic `OpenAI API error: …` branch and surface the underlying SDK error message.
- Show the actual `model` value in the `generate-image` success message instead of the hardcoded `Model: gpt-image-1`.
